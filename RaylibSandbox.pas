...
uses
  Winapi.Windows, Winapi.MultiMon, Winapi.MMSystem, System.SysUtils, System.Classes, System.Math, System.SyncObjs,
  Vcl.Controls, Vcl.Forms, Vcl.Graphics,
  Raylib, RayMath, rlgl, ModelEngine, JoltPhysics;
...
begin
  ...
  begin
    ...
    if FActive then
    begin
      if not FThreadActive then StartThread;
      FPaused := False;
    end
    else
      FPaused := True;
...
begin
  if FTargetFPS <> Value then FTargetFPS := Value;
end;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              repeat QueryPerformanceCounter(FrameEnd);
...
begin
  ...
  case FSpawnShape of
    stSphere: Size := Vector3Create(0.5, 0.5, 0.5);
    stCylinder: Size := Vector3Create(0.5, 1.5, 0.5);
  end;
...
begin
  ...
  begin
    ...
    if (FMousePos.x >= x) and (FMousePos.x <= x + w) and
       (FMousePos.y >= RealY) and (FMousePos.y <= RealY + h) then
    begin
...
begin
  // 1. Check GUI Buttons
  if CheckButton(10, 10, 40, 40) then begin SpawnObjects(30, stBox); Exit; end;
  if CheckButton(60, 10, 40, 40) then begin ClearCubes; Exit; end;
  if CheckButton(110, 10, 40, 40) then begin SpawnObjects(30, stSphere); Exit; end;
  if CheckButton(160, 10, 40, 40) then begin SpawnObjects(30, stCylinder); Exit; end;
...
begin
  ...
  begin
    ...
    if FHUDAnimY > -0.5 then FHUDAnimY := 0;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      BaseColor.r := 190; BaseColor.g := 33; BaseColor.b := 55; BaseColor.a := 255; // Crimson Red
